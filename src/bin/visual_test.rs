/// Visual test tool for N1 encoder/face buttons
/// Shows desktop notifications when you press buttons or rotate the dial
...
// Input processing (same as inputs.rs)
...
fn notify(title: &str, message: &str, urgency: &str) {
...
/// Show a dialog box (blocking, for important events)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("═══════════════════════════════════════════════════════════════");
    println!("  N1 Visual Test Tool");
    println!("═══════════════════════════════════════════════════════════════\n");
    
    // Test notification
    notify("N1 Visual Test", "Starting visual test... Look for notifications!", "low");
    
    let devices = list_devices(&[DeviceQuery::new(65440, 1, AJAZZ_VID, N1_PID)]).await?;
    
    if devices.is_empty() {
        println!("❌ No N1 device found!");
        notify("N1 Error", "Device not found!", "critical");
        return Ok(());
    }
    ...
    println!("✅ Connected!");
    notify("N1 Connected", "Press buttons or rotate dial to see notifications!", "low");
    
    println!("\n📢 Desktop notifications will appear when you:");
    println!("  • Press face buttons (left/right)");
    println!("  • Press the dial");
    println!("  • Rotate the dial left/right");
    ...
    loop {
        match reader.read(None).await {
                ...
                for update in updates {
                    match update {
                        DeviceStateUpdate::ButtonDown(key) => {
                            ...
                            notify(&format!("Button {}", key), "Pressed!", "normal");
                        }
                        DeviceStateUpdate::ButtonUp(key) => {
                            println!("📤 Button {} released", key);
                        }
                        DeviceStateUpdate::EncoderDown(enc) => {
                            println!("🔘 Encoder {} pressed", enc);
                            notify("🔘 Encoder Pressed", "Face button or dial pressed!", "normal");
                        }
                        DeviceStateUpdate::EncoderUp(enc) => {
                            println!("🔘 Encoder {} released", enc);
                        }
                        DeviceStateUpdate::EncoderTwist(enc, val) => {
                            let dir = if val > 0 { "→ Right/CW" } else { "← Left/CCW" };
                            println!("🔄 Encoder {} twisted: {}", enc, dir);
                            notify(&format!("🔄 Dial {}", dir), &format!("Value: {}", val), "normal");
                        }
                    }
                }
            }
            Err(e) => {
                println!("❌ Error: {:?}", e);
                notify("N1 Error", &format!("{:?}", e), "critical");
                break;
            }
        }
    }
